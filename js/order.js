  ...
  let tarifPerFood;
    ...
    if (tarifPerFood === 0 || isNaN(pilihanJumlah)) {
      hasilPesanan.textContent =
        "Pilih menu dan masukan jumlah pesanan yang diinginkan.";
    } else {
      alert("Wait yaa, sedang kami hitung total pembayarannya ^^");
      const totalHarga = pilihanJumlah * tarifPerFood;
      hasilPesanan.textContent = `Total harga untuk ${food}: Rp ${totalHarga}`;
    }
  });
